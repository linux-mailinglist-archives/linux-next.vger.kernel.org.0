...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 614064D9BC0
	for <lists+linux-next@lfdr.de>; Tue, 15 Mar 2022 14:04:50 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S242243AbiCONGA (ORCPT <rfc822;lists+linux-next@lfdr.de>);
        Tue, 15 Mar 2022 09:06:00 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:56294 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S238074AbiCONF7 (ORCPT
        <rfc822;linux-next@vger.kernel.org>); Tue, 15 Mar 2022 09:05:59 -0400
Received: from gandalf.ozlabs.org (mail.ozlabs.org [IPv6:2404:9400:2221:ea00::3])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 94533205FC;
        Tue, 15 Mar 2022 06:04:44 -0700 (PDT)
Received: from authenticated.ozlabs.org (localhost [127.0.0.1])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange ECDHE (P-256) server-signature RSA-PSS (4096 bits) server-digest SHA256)
        (No client certificate requested)
        by mail.ozlabs.org (Postfix) with ESMTPSA id 4KHtsg0Qprz4xQq;
        Wed, 16 Mar 2022 00:04:39 +1100 (AEDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=canb.auug.org.au;
        s=201702; t=1647349479;
        bh=HKtwox+3VUn0yN2f5c3zD5zDDIJeZG8zM46qP16afLY=;
        h=Date:From:To:Cc:Subject:From;
        b=
Date:   Wed, 16 Mar 2022 00:04:37 +1100
From:   Stephen Rothwell <sfr@canb.auug.org.au>
To:     Linux Next Mailing List <linux-next@vger.kernel.org>
Cc:     Linux Kernel Mailing List <linux-kernel@vger.kernel.org>
Subject: linux-next: Tree for Mar 15
Message-ID: <20220316000437.1c8ac482@canb.auug.org.au>
MIME-Version: 1.0
Content-Type: multipart/signed; boundary="Sig_/hGkltXLcDsQ5RzTHxH8MRPZ";
...
--Sig_/hGkltXLcDsQ5RzTHxH8MRPZ
Content-Type: text/plain; charset=US-ASCII
Content-Transfer-Encoding: quoted-printable

Hi all,

Changes since 20220310:

The net-next tree gained a build failure for which I applied a patch.

The tip tree gained conflicts against the net-next and powerpc trees.
It also caused build failures that went away when built from scratch.
I reverted 7 commits.

The ftrace tree gained a conflict against the nfsd tree. Unfortunately,
it was a difficult one, so I used the ftrace tree from next-20220310
for today.

The char-misc tree gained conflicts against the arm-soc and net-next
trees.

The userns tree gained conflicts against the asm-generic and block trees.

The nvdimm tree gained conflicts against the block and powerpc trees.

The sysctl tree gained a conflict against the tip tree.

The folio tree gained conflicts against the erofs, ext4, fscache, nfs
and ubifs-fixes trees.  It also gained a build failure so I used the
version from next-20220310.

The kspp tree gained a conflict against Linus' tree.

The akpm-current tree gained a conflict against the bpf-next tree.

Non-merge commits (relative to Linus' tree): 11744
 11928 files changed, 981172 insertions(+), 258780 deletions(-)
...
Merging origin/master (09688c0166e7 Linux 5.17-rc8)
...
Merging arm-soc-fixes/arm/fixes (7e606edaa0a2 Merge tag 'mvebu-fixes-5.17-2=
...
Merging net/master (e981bc74aefc net: dsa: microchip: add spi_device_id tab=
les)
Merging bpf/master (18b1ab7aa76b xsk: Fix race at socket teardown)
Merging ipsec/master (9a564bccb78a af_key: add __GFP_ZERO flag for compose_=
sadb_supported in function pfkey_register)
...
Merging regulator-fixes/for-linus (2b000c66100f Merge remote-tracking branc=
h 'regulator/for-5.16' into regulator-linus)
Merging spi-fixes/for-linus (827180a2a1a0 Merge remote-tracking branch 'spi=
...
Merging zstd-fixes/zstd-linus (88a309465b3f lib: zstd: clean up double word=
 in comment.)
Merging drm-misc-fixes/for-linux-next-fixes (40ce1121c1d7 drm/mgag200: Fix =
PLL setup for g200wb and g200ew)
...
ice())
Merging arm-soc/for-next (2da5d49b92a5 soc: document merges)
...
roller)
Merging clk/clk-next (93cdab11d346 Merge branch 'clk-apple' into clk-next)
...
Merging xtensa/xtensa-for-next (89b184f980f0 xtensa: make secondary reset v=
ector support conditional)
...
ct I/O support)
Merging fscache/fscache-next (8e26e1e7ae2b afs: Maintain netfs_i_context::r=
emote_i_size)
...
Merging cifs/for-next (4eb628dd74df cifs: during reconnect, update interfac=
e if necessary)
Merging configfs/for-next (84ec758fb2da configfs: fix a race in configfs_{,=
un}register_subsystem())
Merging ecryptfs/next (682a8e2b41ef Merge tag 'ecryptfs-5.13-rc1-updates' o=
f git://git.kernel.org/pub/scm/linux/kernel/git/tyhicks/ecryptfs)
Merging erofs/dev (ab6bfa6e1e22 erofs: refine managed inode stuffs)
...
Merging ext4/dev (f7d6c24543c3 ext4: fix kernel doc warnings)
Merging f2fs/dev (0e18a4058b3e f2fs: use spin_lock to avoid hang)
...
rs in nfs_getattr())
Merging nfsd/for-next (eca663f330dc fs/lock: only call lm_breaker_owns_leas=
...
Merging hwmon-staging/hwmon-next (5d4a2ea96b79 hwmon: (axi-fan-control) Use=
 hwmon_notify_event)
Merging jc_docs/docs-next (ff1368763bb4 Docs: ktap: add code-block type)
Merging v4l-dvb/master (2b891d3980f6 media: xilinx: csi2rxss: Use mipi-csi2=
.h)
Merging v4l-dvb-next/master (a7d36ceef067 media: platform: rename mediatek/=
mtk-jpeg/ to mediatek/jpeg/)
Merging pm/linux-next (bda0a27564ac Merge branches 'pm-cpufreq' and 'pm-cor=
e' into linux-next)
...
Merging net-next/master (bdd6a89de44b nfp: flower: avoid newline at the end=
 of message in NL_SET_ERR_MSG_MOD)
CONFLICT (content): Merge conflict in drivers/hwmon/adt7310.c
Merging bpf-next/for-next (d3b351f65bf4 selftests/bpf: Fix a clang compilat=
ion error for send_signal.c)
...
ent match)
Merging bluetooth/master (29197d4c1f9f Bluetooth: Don't assign twice the sa=
me value)
Merging wireless-next/main (de29aff976d3 Merge tag 'linux-can-next-for-5.18=
-20220313' of git://git.kernel.org/pub/scm/linux/kernel/git/mkl/linux-can-n=
ext)
Merging mtd/mtd/next (5a368fb65ad9 mtd: nand: mxic-ecc: make two read-only =
arrays static const)
...
Merging amdgpu/drm-next (74b121b6a354 drm/radeon: Add HD-audio component no=
tifier support (v2))
Merging drm-intel/for-linux-next (3886a86e7e6c drm/i915/gem: add missing bo=
undary check in vm_access)
...
Merging sound-asoc/for-next (2546018f02f1 Merge remote-tracking branch 'aso=
...
Merging input/next (f28af984e771 Input: mt6779-keypad - add MediaTek keypad=
 driver)
Merging block/for-next (44569689184a Merge branch 'for-5.18/io_uring' into =
for-next)
...
S_DB1XXX boards)
Merging mmc/next (e23b2f54db1d dt-bindings: mmc: sunxi: add Allwinner F1c10=
0s compatible)
...
NOTONIC)
Merging regulator/for-next (9c37731b9501 Merge remote-tracking branch 'regu=
...
finition for mt8186)
Merging spi/for-next (2e295e69be78 Merge remote-tracking branch 'spi/for-5.=
18' into spi-next)
...
Merging tip/auto-latest (6ca31f662ae5 Merge sched/core into tip/master)
...
Applying: fix up for "tracing: Move the defines to create TRACE_EVENTS into=
 their own files"
$ git reset --hard HEAD^
Merging next-20220310 version of ftrace
...
sts)
Merging xen-tip/linux-next (982e4430beb9 drivers/xen: use helper macro __AT=
TR_RW)
Merging percpu/for-next (4e1f82dce05b Merge branch 'for-5.16-fixes' into fo=
r-next)
Merging workqueues/for-next (bc35f7ef9628 workqueue: Convert the type of po=
ol->nr_running to int)
Merging drivers-x86/for-next (854abe25ddb0 platform/x86: amd-pmc: Validate =
...
Merging usb/usb-next (7d2c8a13c925 Merge tag 'thunderbolt-for-v5.18-rc1' of=
...
Merging vfio/next (5b26f2c249b1 vfio/mlx5: Fix to not use 0 as NULL pointer)
Merging staging/staging-next (4cc893176cf6 Merge 5.17-rc8 into staging-next)
...
Merging scsi/for-next (71bb9ab6e351 scsi: ufs: Fix runtime PM messages neve=
r-ending cycle)
...
Applying: Revert "x86: Fix {int3,ibt}_selftest() vs LTO"
Applying: Revert "x86/alternative: Use .ibt_endbr_seal to seal indirect cal=
ls"
Applying: Revert "objtool: Find unused ENDBR instructions"
Applying: Revert "objtool: Validate IBT assumptions"
Applying: Revert "objtool: Add IBT/ENDBR decoding"
Applying: Revert "objtool: Read the NOENDBR annotation"
Applying: Revert "Kbuild: Allow whole module objtool runs"
Merging scsi-mkp/for-next (dc2646417d54 scsi: lpfc: Use kcalloc())
Merging vhost/linux-next (775fa3196050 tools/virtio: fix after premapped bu=
f support)
...
Merging pwm/for-next (ed14d36498c8 pwm: rcar: Simplify multiplication/shift=
 logic)
Merging userns/for-next (f524fbecf1f9 Merge of prlimit-tasklist_lock-for-v5=
...
CONFLICT (content): Merge conflict in kernel/sched/deadline.c
CONFLICT (content): Merge conflict in kernel/sysctl.c
Merging folio/for-next (af564d7369d4 Merge branch 'fs-folio' into for-next)
...
$ git reset --hard HEAD^
Merging next-20220310 version of folio
CONFLICT (modify/delete): arch/nds32/include/asm/pgtable.h deleted in HEAD =
and modified in 0488343ba3dd6eb00b96708ee29c97aa68a00f21.  Version 0488343b=
a3dd6eb00b96708ee29c97aa68a00f21 of arch/nds32/include/asm/pgtable.h left i=
n tree.
CONFLICT (content): Merge conflict in drivers/gpu/drm/drm_cache.c
$ git rm -f arch/nds32/include/asm/pgtable.h
[master 2122ffd573d1] next-20220310/folio
...
Applying: fixup for "net: dsa: felix: configure default-prio and dscp prior=
ities"
Merging akpm-current/current (a48cedacd4b8 ipc/mqueue: use get_tree_nodev()=
 in mqueue_get_tree())
...
Merging akpm/master (b60beb7f5855 kselftest/vm: override TARGETS from argum=
ents)
...

=EeXf
-----END PGP SIGNATURE-----
